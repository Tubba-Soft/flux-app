...
export const ProcessDetailModal: React.FC<Props> = ({ process, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-dark-surface border border-dark-border rounded-2xl max-w-lg w-full p-6 shadow-2xl relative">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex items-center gap-4 mb-5">
          {process.icon_base64 ? (
            ...
            <div className="w-12 h-12 rounded-xl bg-dark-card border border-dark-border flex items-center justify-center text-slate-400 font-bold">
          ...
          <div>
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
              {process.name}
              <span className="text-xs px-2 py-0.5 rounded bg-dark-card border border-dark-border text-slate-400 font-mono">
                PID: {process.pid}
              </span>
            </h3>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-dark-bg p-3 rounded-xl border border-dark-border">
            <span className="text-[11px] text-slate-500 block mb-1">{t.active_connections}</span>
            <span className="text-sm font-bold text-slate-200 font-mono">
              {process.active_streams_count} Sockets
            </span>
          </div>
          <div className="bg-dark-bg p-3 rounded-xl border border-dark-border">
            <span className="text-[11px] text-slate-500 block mb-1">{t.col_total_transfer}</span>
            <span className="text-sm font-bold text-slate-200 font-mono">
        ...
        {/* Action Buttons */}
        <div className="flex items-center justify-between gap-3 pt-4 border-t border-dark-border/40">
          <button
            onClick={handleKill}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 text-xs font-semibold transition"
          >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-dark-card border border-dark-border hover:bg-dark-hover text-xs font-semibold text-slate-200 transition"
            >
